...
contract MastermindGame {
    ...
    struct Turn{
        bytes32 codeHash; //digest of the secret code, published at the beginning of the turn
        ...
        uint[] correctColorAndPosition; //replies of the codeMaker regarding a full match which is a color&position match
        uint[] correctColor; //replies of the codeMaker regarding a partial match which means color ok but wrong position
    ...
    */
     ...
     */
    function publishCodeHash(uint matchId, uint turnId, bytes32 codeDigest) onlyMatchParticipant(matchId) onlyCodeMaker(matchId, turnId) public{ 
     ...
     */
    function guessTheCode(uint matchId, uint turnId, string memory codeProposed) onlyMatchParticipant(matchId) onlyCodeBreaker(matchId, turnId) public{ 
        //The checks regarding the match/turn ids are performed by the modifier
        //Check that this turn is not already finished.
        Match storage m=activeMatches[matchId];
        if((m.turns.length)-1!=turnId)
        ...
        if(!Utils.containsCharsOf(availableColors,codeProposed))
            revert GameUtils.InvalidParameter("codeProposed","Invalid color in the code");
     ...
     */
    function publishFeedback(uint matchId, uint turnId, uint corrPos, uint wrongPosCorrCol) onlyMatchParticipant(matchId) onlyCodeMaker(matchId, turnId) public{
        //The checks regarding the match/turn ids are performed by the modifier
        //Check that this turn is not already finished.
        Match storage m=activeMatches[matchId];
        if((m.turns.length)-1!=turnId)
     ...
     */
    function publishSecret(uint matchId, uint turnId, string memory secret) onlyMatchParticipant(matchId) onlyCodeMaker(matchId, turnId) public{
     ...
     */
    function endTurn(uint matchId, uint turnId) onlyMatchParticipant(matchId) onlyCodeBreaker(matchId, turnId) public{
        Match storage m=activeMatches[matchId];
        ...
        if(turnId<m.turns.length-1) //Case match not suspended or secret not provided
    ...
    */
     ...
     */
    function openDispute(uint matchId, uint turnId, uint feedbackNum) public onlyMatchParticipant(matchId) onlyCodeBreaker(matchId, turnId){
